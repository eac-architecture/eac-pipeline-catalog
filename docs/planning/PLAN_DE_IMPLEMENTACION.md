...
| PC-002 | perfil `packages/nuget` CI | PC-001 | En curso | Tasks .NET nativas reutilizables; lectura Git privada aislada; evento Git pendiente |
| PC-002K | integración Kafka opcional dentro de `eac-nuget-ci` | PC-002 y consumidor Kafka real | Completado | selector `integration-profile: kafka`, Task reutilizable con sidecar fijado y pruebas contra `KAFKA_BOOTSTRAP_SERVERS` |
